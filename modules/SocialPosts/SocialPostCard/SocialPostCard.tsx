...
export const SocialPostCard: React.FC<Props> = ({ data }) => {
    const { mutateAsync: likeTrigger, isLoading: isLiking } = useMutation(likePostFetcher)
    const { mutateAsync: unlikeTrigger, isLoading: isUnliking } = useMutation(unlikePostFetcher)
            ...
            <div className={s.socialPostCardBottom}>
                <ButtonGroup className={s.socialPostActions}>
                    ...
                    <Button
                        ...
                        icon={<><HeartFilled className={clsx({ [s.liked]: isLiked })} /> <span className={s.likeCount}>{likesCount}</span></>}
                    />
                </ButtonGroup>
            </div>
        </article >
